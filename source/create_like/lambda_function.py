...
def lambda_handler(event, context):
    ...
    if "action" not in body or "token" not in body or "message_id" not in body:
        return {
            'statusCode': 400,
            'headers': {'Access-Control-Allow-Origin': '*'},
            'body': 'Missing action, token or message_id'
        }
    ...
    if action in ["like", "dislike"]:
        if not cursor.execute("SELECT type FROM likes WHERE message_id = %s AND user_id = %s", (message_id, user_id)):
            cursor.execute("INSERT INTO likes (message_id, user_id, type) VALUES (%s, %s, %s)", (message_id, user_id, action))
        else:
            tipo_actual = cursor.fetchone()[0]
            if tipo_actual == action:
                cursor.execute("DELETE FROM likes WHERE message_id = %s AND user_id = %s", (message_id, user_id))
            else:
                cursor.execute("UPDATE likes SET type = %s WHERE message_id = %s AND user_id = %s", (action, message_id, user_id))
        conn.commit()
