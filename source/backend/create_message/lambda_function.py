...
def lambda_handler(event, context):
    ...
    if "token" not in body or "message" not in body or "adjunct" not in body:
        return {
            'statusCode': 400,
            'headers': {'Access-Control-Allow-Origin': '*'},
            'body': "Missing token, message or bucket"
        }
    ...
    if user_id:
        cursor.execute("INSERT INTO messages (user_id, message, adjunct) VALUES (%s, %s, %s)",
